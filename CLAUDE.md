...
| 8 | ~~수익률 추이 수집기~~ → **별도 API 서버 프로젝트로 이관** (2026-07-28 결정). 이 repo는 MCP 서버(조회 통로)에 집중. 참고용 설계 메모: holdings.dailyProfitLoss 일일 적재 + 과거는 orders(전 기간 지원)+candles+exchange-rate 근사 재구성. ⚠ 그 서버도 같은 client_id를 쓰면 토큰 1개 정책 때문에 MCP 서버와 토큰을 뺏고 뺏김 — 401 재시도가 흡수하지만 인지할 것 | 🔄 이관 |
| 9 | (추후) 주문·조건주문 6개를 opt-in 플래그 뒤에서 확장 | ❌ 계획 없음 |
...
6. ~~수집기 제작~~ → 별도 API 서버 프로젝트로 이관 (8단계 참고)
